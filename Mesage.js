...
module.exports = (rem, messageRaw) => {
    ...
    message.isStories = message.from == 'status@broadcast'
    message.sender = message.isGroupMsg || message.isStories ? message.key.participant.split(':')[0] : message.fromMe ? rem?.user?.id?.split('@')?.[0]?.split(':')?.[0] + '@s.whatsapp.net' : message.from.split(':')[0]
    ...
    if(message.type == 'messageContextInfo' || message.type == 'senderKeyDistributionMessage' || message.type == 'viewOnceMessage' || message.type == 'viewOnceMessageV2') {
        if(message.type == 'viewOnceMessage' || message.type == 'viewOnceMessageV2') {
            message.type = Object.keys(message?.message?.viewOnceMessage?.message || message?.message?.viewOnceMessageV2?.message)[0]
            message.message = message?.message?.viewOnceMessage?.message || message?.message?.viewOnceMessageV2?.message
    ...
    if(message.type == 'documentWithCaptionMessage') {
    ...
    if((message.message[message.type]?.url?.startsWith('https://web.whatsapp.net'))) message.message[message.type].url = 'https://pps.whatsapp.net' + message.message[message.type].directPath
